...
export default function Logo({ size = 'md', showText = true, className = '', light = false }) {
    const sizes = {
        xs: { w: 24, text: 'text-sm' },
        sm: { w: 32, text: 'text-base' },
        md: { w: 40, text: 'text-xl' },
        lg: { w: 52, text: 'text-2xl' },
        xl: { w: 68, text: 'text-3xl' },
    };
    const s = sizes[size] || sizes.md;
    const id = 'lg' + size;
    const primary = light ? '#C493A8' : '#853953';
    const dark = light ? '#853953' : '#612D53';
    ...
    return (
        <div className={'flex items-center gap-2.5 ' + className}>
            <svg width={s.w} height={s.w} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <circle cx="24" cy="24" r="7.5" fill={light ? 'rgba(255,255,255,0.12)' : 'white'} />
            ...
            {showText && (
                <span className={'font-extrabold tracking-tight ' + s.text}>
                    <span className={light ? 'text-white/90' : 'text-gradient'}>Femina</span>
                    <span className={light ? 'text-white/70' : 'text-brand-primary'}>Sante</span>
                </span>
            )}
        </div>
    );

        ...
        <AppLayout title='Trouver un gynécologue'>
            ...
            <div className='card mb-6 p-5'>
                <div className='grid md:grid-cols-3 gap-4'>
                    <div className='relative md:col-span-1'>
                        ...
                        <input value={query} onChange={e=>setQuery(e.target.value)} placeholder='Rechercher par nom ou spécialité...' className='input-field pl-11'/>
                    </div>
                    <div className='relative'>
                        <MapPin size={18} className='absolute left-4 top-1/2 -translate-y-1/2 text-gray-400'/>
                        <input value={city} onChange={e=>setCity(e.target.value)} placeholder='Ville...' className='input-field pl-11'/>
                    </div>
                    <div className='flex gap-2'>
                        {[{v:'all',l:'All'},{v:'in_person',l:'En cabinet'},{v:'online',l:'Online'},{v:'both',l:'Les deux'}].map(t=>(
                            <button key={t.v} onClick={()=>setType(t.v)} className={'flex-1 px-3 py-3 rounded-xl text-xs font-semibold border-2 transition-all '+(type===t.v?'border-pink-500 text-pink-700 bg-violet-50':'border-gray-100 text-gray-500 hover:border-violet-200')}>{t.l}</button>
                        ))}
                    </div>
                </div>
            </div>
            ...
            {!loading && filtered.length === 0 && (
                <div className='text-center py-16 card'>
                    ...
                    <h3 className='text-lg font-bold text-gray-700 mb-2'>Aucun gynécologue trouvé</h3>
                    <p className='text-gray-500 text-sm'>Essayez d’ajuster vos filtres.</p>
                </div>
            )}
                ...
                {filtered.map(doc=>(
                    <div key={doc.id} className='card card-hover group'>
                        <div className='flex items-start gap-4 mb-4'>
                            <div className='w-16 h-16 rounded-2xl flex items-center justify-center text-white font-bold text-xl flex-shrink-0' style={{background:'linear-gradient(135deg,#1e1b4b,#4f46e5)'}}>
                                {(doc.first_name||'D').charAt(0)}{(doc.last_name||'R').charAt(0)}
                            </div>
                            <div className='flex-1 min-w-0'>
                                <h3 className='font-extrabold text-gray-900 group-hover:text-violet-700 transition-colors'>Dr. {doc.first_name} {doc.last_name}</h3>
                                <p className='text-sm text-violet-700 font-medium capitalize'>{doc.speciality||'Gynécologue'}</p>
                        ...
                        {doc.rating && (
                            <div className='flex items-center gap-2 mb-3'>
                                ...
                                <span className='text-xs text-gray-400'>({doc.avis_count||0} reviews)</span>
                            </div>
                        )}

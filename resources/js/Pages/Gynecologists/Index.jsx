                ...
                {filtered.map(doc => (
                    <div key={doc.id} className='card card-hover group'>
                        <div className='flex items-start gap-4 mb-4'>
                            <div className='w-16 h-16 rounded-2xl flex items-center justify-center text-white font-bold text-xl flex-shrink-0' style={{ background: 'linear-gradient(135deg,#f472b6,#fb7185)' }}>
                                {(doc.first_name || 'D').charAt(0)}{(doc.last_name || 'R').charAt(0)}
                            </div>
                            <div className='flex-1 min-w-0'>
                                <h3 className='font-extrabold text-gray-900 group-hover:text-pink-700 transition-colors'>Dr. {doc.first_name} {doc.last_name}</h3>
                                <p className='text-sm text-pink-700 font-medium capitalize'>{doc.speciality || 'Gynécologue'}</p>
                        ...
                        <Link href={'/gynecologists/' + doc.id} className='btn-primary w-full text-sm flex items-center justify-center gap-2'>

...
const features = [
    { icon: Heart, title: 'Suivi du cycle', desc: 'Suivez votre cycle menstruel, vos symptômes et votre humeur avec des prédictions intelligentes.', color: 'from-pink-500 to-purple-600' },
        ...
        <div className='min-h-screen bg-white overflow-x-hidden'>
            <nav className='fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 glass border-b border-pink-100/40'>
                ...
                <div className='hidden md:flex items-center gap-8 text-sm font-medium text-gray-600'>
                    <a href='#features' className='hover:text-pink-700 transition-colors'>Fonctionnalités</a>
                    <a href='#stats' className='hover:text-pink-700 transition-colors'>Statistiques</a>
                    <a href='#about' className='hover:text-pink-700 transition-colors'>À propos</a>
                </div>
                <div className='flex items-center gap-3'>
                    <Link href='/login' className='text-sm font-semibold text-gray-700 hover:text-pink-700 transition-colors px-4 py-2'>Se connecter</Link>
                ...
                <div className='relative z-10 max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center py-20'>
                    <div>
                        ...
                        <div className='flex flex-wrap gap-4 mb-10'>
                            ...
                            <Link href='/login' className='flex items-center gap-2 px-6 py-3 rounded-xl border-2 border-pink-200 text-pink-700 font-semibold hover:bg-pink-50 transition-all text-base'>Se connecter</Link>
                        </div>
                        <div className='flex flex-wrap gap-6'>
                            {['Gratuit pour commencer','Sans carte bancaire','Annulation à tout moment'].map(t => (
                                <div key={t} className='flex items-center gap-2 text-sm text-gray-500'><CheckCircle size={16} className='text-pink-500'/>{t}</div>
                            ))}
                        </div>
                    </div>
            ...
            <section id='about' className='py-24 px-6 text-center' style={{background:'linear-gradient(135deg,#fdf2f8,#fff8ed)'}}>
                <div className='max-w-3xl mx-auto'>
                    ...
                    <p className='text-gray-500 mb-12'>— Sarah M., utilisatrice depuis 2026</p>
            ...
            <footer className='py-8 px-6 border-t border-pink-100'>
                <div className='max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4'>
                    <Logo size='sm'/>
                    <p className='text-gray-400 text-sm'>© 2026 FeminaSante. Tous droits réservés.</p>
                    <div className='flex gap-6 text-sm text-gray-500'>
                        <Link href='/privacy' className='hover:text-pink-700 transition-colors'>Confidentialité</Link>
                        <Link href='/terms' className='hover:text-pink-700 transition-colors'>Conditions</Link>
                        <Link href='/contact' className='hover:text-pink-700 transition-colors'>Contact</Link>
                    </div>
                </div>
            </footer>
        </div>

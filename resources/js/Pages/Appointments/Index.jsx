                ...
                <div className='flex items-center justify-between'>
                    <div className='flex bg-white rounded-2xl p-1.5 shadow-sm border border-pink-50 gap-1'>
                        {['upcoming','passés'].map(t=>(
                            <button key={t} onClick={()=>setTab(t)} className={'px-5 py-2 rounded-xl text-sm font-semibold capitalize transition-all '+(tab===t?'text-white':'text-gray-500 hover:text-pink-700')} style={tab===t?{background:'linear-gradient(135deg,#f472b6,#ffb6c1)'}:{}}>{t}</button>
                        ))}
                    </div>
                        ...
                        return (
                            <div key={a.id} className='card card-hover flex flex-col md:flex-row md:items-center gap-4'>
                                <div className='flex items-center gap-4 flex-1'>
                                    <div className='w-14 h-14 rounded-2xl flex items-center justify-center text-white font-bold text-lg flex-shrink-0' style={{background:'linear-gradient(135deg,#f472b6,#fb7185)'}}>

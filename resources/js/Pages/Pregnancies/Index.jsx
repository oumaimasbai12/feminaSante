...
export default function Pregnancies() {
    ...
    const currentWeek = active ? (active.current_week || Math.floor((new Date()-new Date(active.start_date))/(1000*60*60*24*7))) : 0;
            ...
            {!loading && showForm && (
                <div className='max-w-lg mx-auto card'>
                    ...
                    <div className='space-y-4'>
                        <div><label className='block text-sm font-semibold text-gray-700 mb-2'>Date des dernières règles</label><input type='date' value={form.start_date} onChange={e=>setForm({...form,start_date:e.target.value})} className='input-field'/></div>
                        ...
                        <div><label className='block text-sm font-semibold text-gray-700 mb-2'>Type de grossesse</label>
                            <select value={form.pregnancy_type} onChange={e=>setForm({...form,pregnancy_type:e.target.value})} className='input-field'>
                                <option value='simple'>Single</option><option value='twins'>Twins</option><option value='triplets'>Triplets</option>
                            </select>
                        </div>

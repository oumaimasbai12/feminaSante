...
export default function Pregnancies() {
    ...
    const trimesters = [{n:'1st Trimester',w:'1-12 weeks',done:currentWeek>=12},{n:'2nd Trimester',w:'13-26 weeks',done:currentWeek>=26},{n:'3rd Trimester',w:'27-40 weeks',done:currentWeek>=40}];

    return (
        <AppLayout title='Pregnancy Tracker'>
            {loading && <div className='flex items-center justify-center h-64'><div className='w-10 h-10 border-4 border-pink-400 border-t-transparent rounded-full animate-spin'></div></div>}

            {!loading && !active && !showForm && (
                <div className='text-center py-20 card max-w-lg mx-auto'>
                    <div className='text-7xl mb-6'>👶</div>
                    <h2 className='text-2xl font-extrabold text-gray-900 mb-3'>Start Tracking Your Pregnancy</h2>
                    ...
                    <button onClick={()=>setShowForm(true)} className='btn-primary flex items-center gap-2 mx-auto'><Plus size={18}/> Start Tracking</button>
                </div>
            )}

            {!loading && showForm && (
                <div className='max-w-lg mx-auto card'>
                    <h3 className='text-xl font-bold text-gray-900 mb-6'>New Pregnancy</h3>
                    <div className='space-y-4'>
                        <div><label className='block text-sm font-semibold text-gray-700 mb-2'>Last Menstrual Period</label><input type='date' value={form.last_menstrual_period} onChange={e=>setForm({...form,last_menstrual_period:e.target.value})} className='input-field'/></div>
                        <div><label className='block text-sm font-semibold text-gray-700 mb-2'>Due Date (optional)</label><input type='date' value={form.due_date} onChange={e=>setForm({...form,due_date:e.target.value})} className='input-field'/></div>
                        <div><label className='block text-sm font-semibold text-gray-700 mb-2'>Pregnancy Type</label>
            ...
            {!loading && active && (
                <div className='space-y-6'>
                    {/* Main tracker card */}
                    <div className='rounded-2xl p-6 text-white relative overflow-hidden' style={{background:'linear-gradient(135deg,#be185d,#7e22ce)'}}>
                        ...
                        <div className='relative z-10 grid md:grid-cols-2 gap-6 items-center'>
                            ...
                            <div className='grid grid-cols-2 gap-3'>
                                {[{l:'Weeks Remaining',v:Math.max(0,40-currentWeek)},{l:'Due Date',v:active.due_date?new Date(active.due_date).toLocaleDateString('en',{month:'short',day:'numeric'}):'TBD'},{l:'Trimester',v:currentWeek<=12?'1st':currentWeek<=26?'2nd':'3rd'},{l:'Type',v:active.pregnancy_type||'Single'}].map(s=>(
                            ...
                            {trimesters.map((t,i)=>(
                                <div key={t.n} className={'p-4 rounded-2xl border-2 transition-all '+(i===0&&currentWeek<=12?'border-pink-400 bg-pink-50':i===1&&currentWeek>12&&currentWeek<=26?'border-purple-400 bg-purple-50':i===2&&currentWeek>26?'border-indigo-400 bg-indigo-50':'border-gray-100 bg-gray-50')}>
                                    <div className='flex items-center gap-2 mb-2'>
                                        {t.done?<CheckCircle size={18} className='text-green-500'/>:<div className={'w-5 h-5 rounded-full border-2 '+(i===0&&currentWeek<=12?'border-pink-400':i===1?'border-purple-400':'border-indigo-400')}></div>}
                    ...
                    <button onClick={()=>setShowForm(true)} className='btn-primary flex items-center gap-2'><Plus size={18}/> Add New Pregnancy</button>
                </div>
            )}
        </AppLayout>
    );

        ...
        <AppLayout title='Assistant Sante IA'>
            <div className='flex flex-col h-full' style={{ height: 'calc(100vh - 130px)' }}>
                <div className='card flex flex-col h-full overflow-hidden p-0'>
                    {/* Header */}
                    <div className='flex items-center gap-4 p-5 border-b border-pink-50' style={{ background: 'linear-gradient(135deg,#f472b6,#fb7185)' }}>
                        ...
                        {messages.map((m, i) => (
                            <div key={i} className={'flex gap-3 ' + (m.role === 'user' ? 'flex-row-reverse' : '')}>
                                ...
                                <div className={'max-w-[75%] ' + (m.role === 'user' ? 'items-end' : 'items-start') + ' flex flex-col gap-1'}>
                                    <div className={'px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-sm ' + (m.role === 'user' ? 'text-white rounded-tr-sm' : 'bg-white text-gray-700 border border-pink-50 rounded-tl-sm')} style={m.role === 'user' ? { background: 'linear-gradient(135deg,#f472b6,#ffb6c1)' } : {}}>
                                        {m.content}
                                    </div>
                    ...
                    <div className='p-4 border-t border-pink-50 bg-white/80'>
                        <form onSubmit={e => { e.preventDefault(); send(); }} className='flex gap-3'>
                            ...
                            <button type='submit' disabled={!input.trim() || loading} className='w-12 h-12 rounded-xl flex items-center justify-center text-white transition-all disabled:opacity-50' style={{ background: 'linear-gradient(135deg,#f472b6,#ffb6c1)' }}><Send size={18} /></button>
                        </form>
                    </div>
                </div>
            </div>

...
const suggestions = [
    ...
    'What foods help during menstruation?',
        ...
        <AppLayout title='AI Health Assistant'>
            <div className='flex flex-col h-full' style={{height:'calc(100vh - 130px)'}}>
                <div className='card flex flex-col h-full overflow-hidden p-0'>
                    {/* Header */}
                    <div className='flex items-center gap-4 p-5 border-b border-pink-50' style={{background:'linear-gradient(135deg,#be185d,#7e22ce)'}}>
                    ...
                    <div className='flex-1 overflow-y-auto p-5 space-y-4'>
                        {messages.map((m, i) => (
                            <div key={i} className={'flex gap-3 ' + (m.role==='user'?'flex-row-reverse':'')}>
                                ...
                                <div className={'max-w-[75%] ' + (m.role==='user'?'items-end':'items-start') + ' flex flex-col gap-1'}>
                                    <div className={'px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-sm ' + (m.role==='user'?'text-white rounded-tr-sm':'bg-white text-gray-700 border border-pink-50 rounded-tl-sm')} style={m.role==='user'?{background:'linear-gradient(135deg,#EC4899,#A855F7)'}:{}}>
                                        {m.content}
                                    </div>
                        ...
                        {loading && (
                            <div className='flex gap-3'>
                                ...
                                <div className='bg-white border border-pink-50 rounded-2xl rounded-tl-sm px-5 py-3 shadow-sm'>
                    ...
                    {messages.length <= 2 && (
                        <div className='px-5 pb-4'>
                            ...
                            <div className='flex flex-wrap gap-2'>
                                {suggestions.map(s=>(
                                    <button key={s} onClick={()=>send(s)} className='px-3 py-1.5 rounded-full text-xs font-medium bg-pink-50 text-pink-700 border border-pink-100 hover:bg-pink-100 hover:border-pink-300 transition-all'>{s}</button>
                                ))}
                            </div>
                        </div>
                    )}
                    ...
                    {/* Input */}
                    <div className='p-4 border-t border-pink-50 bg-white/80'>
                        <form onSubmit={e=>{e.preventDefault();send();}} className='flex gap-3'>
                            <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} placeholder='Ask me anything about your health...' className='input-field flex-1 py-3' disabled={loading}/>
                            <button type='submit' disabled={!input.trim()||loading} className='w-12 h-12 rounded-xl flex items-center justify-center text-white transition-all disabled:opacity-50' style={{background:'linear-gradient(135deg,#EC4899,#A855F7)'}}><Send size={18}/></button>
                        </form>
                    </div>
                </div>
            </div>

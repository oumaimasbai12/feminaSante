...
const moods = [{v:'happy',l:'Happy',icon:'😊'},{v:'neutral',l:'Neutral',icon:'😐'},{v:'sad',l:'Sad',icon:'😔'},{v:'anxious',l:'Anxious',icon:'😟'},{v:'energetic',l:'Energetic',icon:'🥳'},{v:'tired',l:'Tired',icon:'😴'}];
const flows = [{v:'light',l:'Light',c:'bg-pink-200'},{v:'medium',l:'Medium',c:'bg-pink-400'},{v:'heavy',l:'Heavy',c:'bg-rose-500'},{v:'spotting',l:'Spotting',c:'bg-pink-100'}];
...
export default function Cycles() {
    ...
    const phaseColors = { period: 'bg-rose-400 text-white', ovulation: 'bg-purple-400 text-white', fertile: 'bg-fuchsia-300 text-white', today: 'ring-2 ring-pink-500' };

    return (
        <AppLayout title='Cycle Tracker'>
                ...
                <div className='lg:col-span-2'>
                    <div className='card'>
                        ...
                        <div className='flex items-center justify-between mb-6'>
                            <button onClick={()=>{if(month===0){setMonth(11);setYear(y=>y-1);}else setMonth(m=>m-1);}} className='p-2 rounded-xl hover:bg-pink-50 text-gray-600 hover:text-pink-600 transition-colors'><ChevronLeft size={20}/></button>
                            <h2 className='text-lg font-bold text-gray-900'>{MONTHS[month]} {year}</h2>
                            <button onClick={()=>{if(month===11){setMonth(0);setYear(y=>y+1);}else setMonth(m=>m+1);}} className='p-2 rounded-xl hover:bg-pink-50 text-gray-600 hover:text-pink-600 transition-colors'><ChevronRight size={20}/></button>
                        </div>
                        {/* Day headers */}
                        ...
                        <div className='grid grid-cols-7 gap-1'>
                            ...
                            {[...Array(daysInMonth)].map((_,i)=>{
                                ...
                                return (
                                    <button key={d} onClick={()=>setSelected(dateStr)} className={'relative aspect-square flex items-center justify-center rounded-xl text-sm font-medium transition-all duration-200 hover:scale-105 '+(isToday?'ring-2 ring-pink-500 ring-offset-1 ':'' )+(cycleInfo?' bg-rose-400 text-white shadow-sm':(selected===dateStr?'bg-pink-100 text-pink-700':'text-gray-700 hover:bg-pink-50'))}>
                                        {d}
                        ...
                        <div className='flex flex-wrap gap-4 mt-6 pt-4 border-t border-pink-50'>
                            {[{c:'bg-rose-400',l:'Period'},{c:'bg-purple-400',l:'Ovulation'},{c:'bg-fuchsia-300',l:'Fertile window'},{c:'ring-2 ring-pink-500 bg-white',l:'Today'}].map(i=>(
                            ...
                            <div className='space-y-3'>
                                ...
                                <div>
                                    ...
                                    <div className='grid grid-cols-2 gap-2'>
                                        {flows.map(f=>(
                                            <button key={f.v} onClick={()=>setForm({...form,flow_intensity:f.v})} className={'px-3 py-2 rounded-xl text-xs font-semibold border-2 transition-all '+(form.flow_intensity===f.v?'border-pink-400 bg-pink-50 text-pink-700':'border-gray-100 text-gray-500 hover:border-pink-200')}>{f.l}</button>
                                        ))}
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className='grid grid-cols-3 gap-2'>
                                        {moods.map(m=>(
                                            <button key={m.v} onClick={()=>setForm({...form,mood:m.v})} className={'px-2 py-2 rounded-xl text-xs font-medium border-2 transition-all text-center '+(form.mood===m.v?'border-pink-400 bg-pink-50':'border-gray-100 hover:border-pink-200')}>
                                ...
                                <div>
                                    ...
                                    <textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} rows={2} placeholder='How are you feeling?' className='input-field text-sm resize-none'/>
                                </div>
                        ...
                        {cycles.length === 0 ? (
                            ...
                            <div className='space-y-3'>
                                {cycles.slice(0,5).map((c,i)=>(
                                    <div key={i} className='flex items-center gap-3 p-3 rounded-xl bg-pink-50/50 hover:bg-pink-50 transition-colors'>
                                        <div className='w-9 h-9 rounded-xl bg-rose-400 flex items-center justify-center'><Droplets size={16} className='text-white'/></div>
                                        <div>

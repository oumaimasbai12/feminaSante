...
const flows = [{v:'light',l:'Léger',c:'bg-pink-200'},{v:'medium',l:'Moyen',c:'bg-pink-400'},{v:'heavy',l:'Abondant',c:'bg-pink-600'}];
...
export default function Cycles() {
    ...
    const phaseColors = { period: 'bg-pink-500 text-white', ovulation: 'bg-amber-500 text-white', fertile: 'bg-indigo-300 text-white', today: 'ring-2 ring-pink-500' };
                ...
                <div className='lg:col-span-2'>
                    <div className='card'>
                        ...
                        <div className='flex items-center justify-between mb-6'>
                            <button onClick={()=>{if(month===0){setMonth(11);setYear(y=>y-1);}else setMonth(m=>m-1);}} className='p-2 rounded-xl hover:bg-pink-50 text-gray-600 hover:text-pink-700 transition-colors'><ChevronLeft size={20}/></button>
                            <h2 className='text-lg font-bold text-gray-900'>{MONTHS[month]} {year}</h2>
                            <button onClick={()=>{if(month===11){setMonth(0);setYear(y=>y+1);}else setMonth(m=>m+1);}} className='p-2 rounded-xl hover:bg-pink-50 text-gray-600 hover:text-pink-700 transition-colors'><ChevronRight size={20}/></button>
                        </div>
                        {/* Day headers */}
                        ...
                        <div className='grid grid-cols-7 gap-1'>
                            ...
                            {[...Array(daysInMonth)].map((_,i)=>{
                                ...
                                return (
                                    <button key={d} onClick={()=>setSelected(dateStr)} className={'relative aspect-square flex items-center justify-center rounded-xl text-sm font-medium transition-all duration-200 hover:scale-105 '+(isToday?'ring-2 ring-pink-500 ring-offset-1 ':'' )+(cycleInfo?' bg-pink-500 text-white shadow-sm':(selected===dateStr?'bg-pink-100 text-pink-700':'text-gray-700 hover:bg-pink-50'))}>
                                        {d}
                        ...
                        <div className='flex flex-wrap gap-4 mt-6 pt-4 border-t border-pink-50'>
                            {[{c:'bg-pink-500',l:'Règles'},{c:'bg-amber-500',l:'Ovulation'},{c:'bg-indigo-300',l:'Fenêtre fertile'},{c:'ring-2 ring-pink-500 bg-white',l:'Aujourd\'hui'}].map(i=>(
                            ...
                            <div className='space-y-3'>
                                {cycles.slice(0,5).map((c,i)=>(
                                    <div key={i} className='flex items-center gap-3 p-3 rounded-xl bg-pink-50/50 hover:bg-pink-50 transition-colors'>
                                        <div className='w-9 h-9 rounded-xl bg-pink-500 flex items-center justify-center'><Droplets size={16} className='text-white'/></div>
                                        <div>
